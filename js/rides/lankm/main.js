...
import {Teacups} from "./Teacups.js";
import {loadFBX} from "./Utils.js";
import {CarnivalGame} from "./CarnivalGame.js";
...
function initTeacupsExhibit() {
    let park = new Teacups(userRig, 3, 6, 1, 1);
    park.position.z = 1;
    animatedObjects.push(park);
    scene.add(park);
}
...
function initExhibit2(userRig) {
    ...
    // Load the model
    loadFBX('extern/models/lambo/Lamborghini_Aventador.fbx', obj => {
...
function initCarnivalGame(userRig) {
    carnivalGame = new CarnivalGame(userRig, 0, 1);
    carnivalGame.position.z = -5;
    animatedObjects.push(carnivalGame);
    scene.add(carnivalGame);
}
