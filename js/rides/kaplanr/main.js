...
import * as RYAN from './ryanExhibit.js';
import * as GAME from './ryanGame.js';
...
function initRyanGame(userRig){

  var game = new GAME.ryanGame(userRig, animatedObjects, 5, 5);
  game.rotation.y = (THREE.Math.degToRad(-90));
  game.position.x = 5;
  game.position.z = -10;
  scene.add(game);

  var game2 = new GAME.ryanGame(userRig, animatedObjects, 10, 10);
  game2.rotation.y = (THREE.Math.degToRad(90));
  game2.position.x = -5;
  game2.position.z = -10;
  scene.add(game2);

  var game3 = new GAME.ryanGame(userRig, animatedObjects, 1, 1);
  game3.rotation.y = (THREE.Math.degToRad(180));
  game3.position.z = 5;
  scene.add(game3);
}

function initRyanExhibit(userRig){

  // Add landing platform for the exhibit.
  var ryan1 = new RYAN.ryanExhibit(userRig, animatedObjects, 5,5);
  ryan1.position.z = -40;
  scene.add(ryan1);

  var ryan2 = new RYAN.ryanExhibit(userRig, animatedObjects, 1,9);
  ryan2.position.z = -45;
  ryan2.position.x = -30;
  ryan2.rotation.y = (THREE.Math.degToRad(-45));
  scene.add(ryan2);

  var ryan3 = new RYAN.ryanExhibit(userRig, animatedObjects, 9,1);
  ryan3.position.z = -45;
  ryan3.position.x = 30;
  ryan2.rotation.y = (THREE.Math.degToRad(45));
  scene.add(ryan3);
...
function initExhibit4(userRig){
    ...
    var buttons = [new GUIVR.GuiVRButton("y", 3, 2.5, 6, false,
		   ...
		   new GUIVR.GuiVRButton("angle", 26, 0, 90, false,
					 function(x){
					     light.angle = THREE.Math.degToRad(x);
					     shadowCamera.camera.fov = x;
					 }),
		   new GUIVR.GuiVRButton("far", 3, 0, 10, true,
					 function(x){
...
function initExhibits(userRig){
    ...
    initRyanExhibit(userRig);
    initRyanGame(userRig);
}
