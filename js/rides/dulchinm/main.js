...
import * as TEACUPS from './Teacups.js';
...
function initBalloonDarts(userRig,ridePosX,ridePosZ,rideRotationDegree,balloons,gravityModifier,balloonRadius){
	var ride = new BALLOON.Balloon(userRig,balloons,gravityModifier,balloonRadius);
	//animatedObjects.push(ride);
	ride.rotation.y = THREE.Math.degToRad(rideRotationDegree);
	ride.position.z = ridePosZ;
	ride.position.x = ridePosX;
	scene.add(ride);
}

function initTeacups(userRig,astroids,speed,ridePosX,ridePosZ,rideRotationDegree){
	var ride = new TEACUPS.Teacups(userRig,astroids,speed);
	animatedObjects.push(ride);
	ride.rotation.y = THREE.Math.degToRad(rideRotationDegree);
    ride.position.z = ridePosZ;
    ride.position.x = ridePosX;
...
function initExhibits(userRig){
    ...
    //initBalloonDarts(userRig);
	initBalloonDarts(userRig,3,-7,270,5,1,0.2)
	initBalloonDarts(userRig,3,-27,270,2,3,0.1)
	initBalloonDarts(userRig,3,-47,270,10,0,0.3)

    initTeacups(userRig,5,1,-2,-7,90);
    initTeacups(userRig,50,2,-2,-27,90);
    initTeacups(userRig,1,-20,-2,-47,90);
}
