...
export class TeaCupPlatform extends GUIVR.GuiVR{
  constructor(userRig){
  super();

  // creates a single teacup
  var points = [];
  for ( var i = 0; i < 10; i ++ ) {
    points.push( new THREE.Vector2( Math.sin( i * 0.2 ) * 10 + 5, ( i - 5 ) * 2 ) );
  }

  // create a single handle of the TeaCup
  var curve = new THREE.CatmullRomCurve3([
   ...
   new THREE.Vector3( 0, 0, 0 ),new THREE.Vector3( -5, 0, 5 )
    ...
    // creates first TeaCup
   var geometry = new THREE.LatheGeometry( points );
   var material = new THREE.MeshBasicMaterial( { color: 0x455fbf} );
   var lathe = new THREE.Mesh( geometry, material);
   lathe.scale.x = 0.10;
   lathe.scale.y=0.15;
   lathe.scale.z=0.10;
   lathe.position.y = 1.30;
   lathe.receiveShadow = true;

   // creates the handle of the teacup
   var geometry = new THREE.TubeGeometry(curve, 20, 2, 3, false);
   var material = new THREE.MeshBasicMaterial( { color: 0xd3ffce} );
   var handle = new THREE.Mesh( geometry, material);
   handle.position.y =6;
   handle.position.x = 18;
   handle.position.z = -7;

   handle.rotation.y= THREE.Math.degToRad(200);
   handle.rotation.z= THREE.Math.degToRad(180);
   lathe.add(handle);


   // add the lathe to userRig
  this.add(lathe);
