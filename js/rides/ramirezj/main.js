...
import * as TEACUP from './TeaCup.js';
import * as WHACKAMOLE from './WhackAMole.js';
...
function initExhibits(userRig){
    ...
    // allows users to change start speed, maxspeed, and the increase speed of
    // platform speed and cup speed
    exhibit.add(new TEACUP.initExhibit5(userRig,1,15,0.01,0.05));
    animatedObjects.push(exhibit);
    scene.add(exhibit);


    var carnival = new THREE.Group();
    carnival.add(new WHACKAMOLE.whackAMole(userRig));
    animatedObjects.push(carnival);
    scene.add(carnival);
}
