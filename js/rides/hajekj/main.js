...
function initExhibit2(userRig){
  ...
  var exhibit = new THREE.Group();
  let up = new USER.UserPlatform(userRig);
  up.onLand = () => {
    ...
    (dt) => {
      ...
      && (controller.t - controller.lastFire >= 0.3
        || controller.lastFire == undefined)){
      controller.lastFire = controller.t;
      
      // Create new projectile and set up motion.
      /*
      let proj = new THREE.Group();
      proj.add(new THREE.Mesh(
        new THREE.CubeGeometry(0.5, 0.5, 2),
      ...
      */
      ...
      proj.position.setFromMatrixPosition(controller.matrixWorld);
      controller.getWorldQuaternion(proj.quaternion);
      scene.add(proj);

      animatedObjects.push(proj);
      proj.setAnimation(
        (dt) => {
        if (proj.t == undefined){
          proj.t = 0;
        ...
        if (proj.t > 20){
          scene.remove(proj);
          animatedObjects.splice(animatedObjects.indexOf(proj), 1);
        }
        }
      );
      }
...
function initMyRide(userRig, relPos, relRot, radius, seatsAmt) {
  let myRide = new Ride(userRig, relPos, relRot, radius, seatsAmt);
  animatedObjects.push(myRide);
  scene.add(myRide);
}

function initMyGame(userRig, relPos, relRot, ducksAmt, duckBaseSpeed, projRadius, projSpeed, rateOfFire) {
  let myGame = new Game(userRig, relPos, relRot, ducksAmt, duckBaseSpeed, projRadius, projSpeed, rateOfFire);
...
function initExhibits(userRig){
  ...
  // Initialize my rides.
  let radius = [10, 5, 10];
  let seatsAmt = [5, 2, 100];
  initMyRide(userRig, new THREE.Vector3(-20, 0,-5), new THREE.Vector3(0, Math.PI, 0), radius[0], seatsAmt[0]);
  initMyRide(userRig, new THREE.Vector3(0, 0, -5), new THREE.Vector3(0, Math.PI, 0), radius[1], seatsAmt[1]);
  initMyRide(userRig, new THREE.Vector3(20, 0, -5), new THREE.Vector3(0, Math.PI, 0), radius[2], seatsAmt[2]);

  // Initialize my games.
  let ducksAmt = [2, 5, 10];
  let duckBaseSpeed = [1, 5, 10];
  let projRadius = [1, 3, 5];
  let projSpeed = [1, 2, 5];
  let rateOfFire = [1, 3, 5];
  initMyGame(userRig, new THREE.Vector3(-10, 0, -5), new THREE.Vector3(0, 0, 0), ducksAmt[0], duckBaseSpeed[0], projRadius[0], projSpeed[0], rateOfFire[0]);
  initMyGame(userRig, new THREE.Vector3(0, 0, -5), new THREE.Vector3(0, 0, 0), ducksAmt[1], duckBaseSpeed[1], projRadius[1], projSpeed[1], rateOfFire[1]);
  initMyGame(userRig, new THREE.Vector3(10, 0, -5), new THREE.Vector3(0, 0, 0), ducksAmt[2], duckBaseSpeed[2], projRadius[2], projSpeed[2], rateOfFire[2]);
}
