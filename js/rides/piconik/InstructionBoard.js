...
// Version: Winter 2020
// Project 1: InstructionBoard.
// Instructionboard, displays text instructions to assist user.
...
export function instructionWrite(msg){
    instructionOutput += msg + "\n";

    // Determine the lines of text that appear.
    var lines = instructionOutput.split('\n');
    lines = lines.slice(-NUM_LINES-1, -1);
    instructionOutput = lines.join("\n") + "\n";

    // Update the display of all scoreboards.
    for (var i = 0; i < instructionBoards.length; i++){
        instructionBoards[i].updateTexture();
    }
}

// Class for displaying score text in VR.  Specialized THREE.Group.
export class InstructionBoard extends GUIVR.GuiVR {
